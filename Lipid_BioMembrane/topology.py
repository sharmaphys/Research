...
def radius_of_gyration(coord):
  ...
  #translate the com to origin
   ...
   rgSquared = float(np.sum(np.sum(recenteredCom**2, axis=1))/N)
...
def anisotropy(coord):
   ...
   It reflects hte symmetry and dimensionality of a polymer 
   conformation and lies between values 0 and 1. 
  ...
  #translate the coordinates to body center of mass
   ...
   gyrationTensor /= mi*len(coord)
   
   eig_vals = np.linalg.eigvalsh(gyrationTensor)
   # returned eigen values are in ascending order
